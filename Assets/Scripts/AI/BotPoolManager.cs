...
namespace AI
{
    public class BotPoolManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            foreach (var botDeath in botPool.Select(bot => bot.GetComponent<BotDeath>())
                         .Where(botDeath => botDeath != null)) botDeath.OnBotDeath += HandleBotCollected;
        }
        ...
        private void InitializePool()
        {
            for (var i = 0; i < poolSize; i++)
            {
                ...
                var botCanvas = botInstance.GetComponentInChildren<Canvas>();
                
                botCanvas.AddComponent<FaceCamera>().MainCamera = mainCamera;
                
                var botScore = botInstance.GetComponent<BotScore>();
                if (botScore != null)
                {
                    botScore.InitializeScore(randomSize * scoreSizeMultiplier);
                }

                var botRank = botInstance.GetComponent<RankScore>();
                rankContainer.RankScores.Add(botRank);
        ...
        private void ReturnBotToPool(GameObject bot)
        {
            ...
            var botScore = bot.GetComponent<BotScore>();
            if (botScore != null)
            {
                botScore.InitializeScore(randomSize * scoreSizeMultiplier);
            }
